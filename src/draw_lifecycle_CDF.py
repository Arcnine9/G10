...
# 移除 inactive_time 中的 NaN 值
df = df.dropna(subset=['inactive_time'])

# 确保 inactive_time 为非负值
...
plt.plot(cdf, inactive_times, lw=2, marker='o')  # 使用圆点标记每个数据点
plt.xscale('linear')  # 使用线性横轴
plt.yscale('log')  # 纵轴为对数
plt.xlabel('CDF (%)')
plt.ylabel('Tensor Inactive Period Total (µs)')
plt.title('Distribution of Tensor Inactive Period Total Lengths')
plt.grid(True, which='both', ls='--', alpha=0.5)
plt.savefig('tensor_inactive_periods_cdf.png', dpi=300, bbox_inches='tight')  # 保存图像
plt.show()